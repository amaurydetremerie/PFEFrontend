...
export class AnnonceSignaleComponent implements OnInit {
  ...
  constructor(private service: OffersService, private router: Router, private toastr: ToastrService) {}
  ...
  resetSignalements(id: string){
    ...
    this.service.resetSignalements(parseInt(id)).subscribe(result => window.location.reload(),
      err => {
        this.toastr.error(err);
      });
  }
  ...
  deleteOffer(id: string): void {
    // tslint:disable-next-line:radix
    this.service.deleteOffer(parseInt(id)).subscribe(result => window.location.reload(),
      err => {
        this.toastr.error(err);
      });
  }
